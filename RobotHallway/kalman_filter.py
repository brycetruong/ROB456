...
class KalmanFilter:
    ...
    def update_belief_distance_sensor(self, robot_sensors: RobotSensors, dist_reading:float):
        ...
        # YOUR CODE HERE
        sensor_sigma = robot_sensors.distance_wall_sigma
        
        # NOTE: For this specific assignment, use Linear approximation (no squares)
        # K = sigma / (sigma + sensor_sigma)
        K = self.sigma / (self.sigma + sensor_sigma)
        
        # Update Mean: mu = mu + K * (z - mu)
        self.mu = self.mu + K * (dist_reading - self.mu)
        
        # Update Sigma: sigma = (1 - K) * sigma
    ...
    def update_continuous_move(self, 
        ...
        # Update Sigma (Prediction) - Linear addition (no squares)
